...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_random_by_category | image | search"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NekosbestSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "nekosbest_list",
		Description: "List records from Nekosbest. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "nekosbest_load",
		Description: "Load a single record from Nekosbest. " +
...
func runOp(client *sdk.NekosbestSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NekosbestSDK, name string) (sdk.NekosbestEntity, error) {
	switch strings.ToLower(name) {
	case "get_random_by_category":
		return client.GetRandomByCategory(nil), nil
	case "image":
