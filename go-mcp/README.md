...
[MCP](https://modelcontextprotocol.io) server exposing the Nekosbest SDK as
two agent tools — `nekosbest_list` and `nekosbest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nekosbest-mcp)
make build

# 2. Provide credentials via the environment
export NEKOSBEST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nekosbest \
  -- /absolute/path/to/nekosbest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nekosbest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nekosbest_list: first page of records
{ "entity": "get_random_by_category" }
{ "entity": "get_random_by_category", "query": { } }

// nekosbest_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nekosbest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nekosbest \
     -- "$PWD"/dist/*/nekosbest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nekosbest_list` and `nekosbest_load` tools now appear
   in new sessions. Ask the agent to *"list get_random_by_category using nekosbest"*
   and it calls `nekosbest_list` with `{"entity":"get_random_by_category"}`.
...
```sh
export NEKOSBEST_APIKEY=sk_live_xxx            # API key
export NEKOSBEST_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `nekosbest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nekosbest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEKOSBEST_APIKEY` | API key sent with every request. |
| `NEKOSBEST_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

get_random_by_category | image | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./nekosbest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nekosbest_load","arguments":{"entity":"image","query":{"id":1}}}}'
```
