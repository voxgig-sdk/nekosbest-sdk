...
//     (\`new NekosbestSDK(...)\` / \`NekosbestSDK.test(...)\` become
//     \`NekosbestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_random_by_category":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NekosbestSDK(...)\` and any
// \`NekosbestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NekosbestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NekosbestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
