...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NEKOSBEST_TEST_LIVE ||
    'TRUE' === process.env.NEKOSBEST_TEST_OVERRIDE
  ) {
  ...
  m.NEKOSBEST_TEST_EXPLAIN = process.env.NEKOSBEST_TEST_EXPLAIN || m.NEKOSBEST_TEST_EXPLAIN
