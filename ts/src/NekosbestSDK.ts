...
class NekosbestSDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('NekosbestSDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('NekosbestSDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('NekosbestSDK: graphql: ' +
  ...
  GetRandomByCategory(entopts?: Record<string, any>) {
    const self = this
    return new GetRandomByCategoryEntity(self, entopts)
  }
  ...
  static test(testoptsarg?: any, sdkoptsarg?: any) {
    ...
    const testsdk = new NekosbestSDK(sdkopts)
    testsdk._mode = 'test'
  ...
  tester(testopts?: any, sdkopts?: any) {
    return NekosbestSDK.test(testopts, sdkopts)
  }


  toJSON() {
    return { name: 'Nekosbest' }
  }

  toString() {
    return 'Nekosbest ' + this._utility.struct.jsonify(this.toJSON())
  }
