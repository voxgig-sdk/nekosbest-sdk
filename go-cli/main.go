// Command Nekosbest-cli is a CLI + REPL for the Nekosbest SDK,
...
const prompt = "nekosbest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_random_by_category image search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NEKOSBEST_APIKEY (api key), NEKOSBEST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NEKOSBEST_APIKEY carries the API key and
	// NEKOSBEST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NEKOSBEST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NEKOSBEST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNekosbestSDK(opts)
