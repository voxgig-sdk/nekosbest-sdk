...
boru-driven command-line client **and** interactive REPL for the Nekosbest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nekosbest-cli)
make build
...
./nekosbest-cli list image

# 5. Override the API base URL for a single call
NEKOSBEST_BASE=https://api.example.com ./nekosbest-cli list get_random_by_category

# 6. No arguments -> interactive REPL
./nekosbest-cli
nekosbest> list get_random_by_category
nekosbest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nekosbest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nekosbest-cli list get_random_by_category
   ```
...
Run with no arguments to open a REPL (prompt `nekosbest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_by_category`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nekosbest-cli`. |
...
(`./nekosbest-cli <expr>`) and interactively (the REPL), and expressions compose the
