...
class FtCtrl extends NekosbestControl
{
...
class FtClient
{
    ...
    private NekosbestUtility $utility;

    public function __construct(NekosbestUtility $utility, array $options)
    {
    ...
    public function get_utility(): NekosbestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NekosbestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NekosbestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NekosbestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NekosbestConfig::make_feature($name);
    ...
    private static function buildUrl(NekosbestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NekosbestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NekosbestResponse($fetched);
        }
    ...
    private function fail(NekosbestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NekosbestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
