...
// Nekosbest SDK GetRandomByCategory entity
...
class GetRandomByCategoryEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_random_by_category";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetRandomByCategoryEntity($this->_client, $opts);
    }

    /**
     * @param GetRandomByCategory|array $args GetRandomByCategory data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NekosbestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetRandomByCategory|array The current GetRandomByCategory data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetRandomByCategory fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NekosbestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetRandomByCategory fields).
     */
     ...
     *
     * @param GetRandomByCategoryListMatch|array|null $reqmatch Match filter (any subset
     *   of GetRandomByCategory fields) as an assoc-array; GetRandomByCategoryListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetRandomByCategory[]|array A list of GetRandomByCategory items as assoc-arrays at
     *   the SDK boundary; throws NekosbestError on failure (item-5 convention).
     */
