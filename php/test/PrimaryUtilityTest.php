...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        NekosbestSDK $client,
        NekosbestUtility $utility
    ): NekosbestContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new NekosbestContext($ctxmap, null);
    ...
    private static function fixctx(NekosbestContext $ctx, NekosbestSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?NekosbestError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NekosbestError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new NekosbestResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new NekosbestBaseFeature();
    ...
    {
        $hook_client = NekosbestSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends NekosbestBaseFeature {
            public $hook_fn = null;
            public function TestHook(NekosbestContext $ctx): void
            {
    ...
    {
        $init_client = NekosbestSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends NekosbestBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new NekosbestSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new NekosbestSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = NekosbestSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = NekosbestSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof NekosbestError) ? $result->err->msg : (string)$result->err;
