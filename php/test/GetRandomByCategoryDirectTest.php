...
class GetRandomByCategoryDirectTest extends TestCase
{
    public function test_direct_list_get_random_by_category(): void
    {
        $setup = get_random_by_category_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-get_random_by_category", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["get_random_by_category01"] as $_liveKey) {
