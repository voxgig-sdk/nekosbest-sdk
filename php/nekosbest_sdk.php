...
class NekosbestSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NekosbestHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NekosbestHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NekosbestFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NekosbestFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NekosbestUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NekosbestHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NekosbestHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NekosbestHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NekosbestError($op . "_allow",
                "NekosbestSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NekosbestHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NekosbestError("graphql_error",
                "NekosbestSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetRandomByCategory($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_random_by_category === null) {
                $this->_get_random_by_category = new GetRandomByCategoryEntity($this, null);
            }
            return $this->_get_random_by_category;
        }
        return new GetRandomByCategoryEntity($this, $data);
    }
