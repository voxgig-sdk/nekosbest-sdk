...
require_relative 'Nekosbest_types'
...
class NekosbestSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NekosbestHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NekosbestHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NekosbestFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NekosbestUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NekosbestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NekosbestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NekosbestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NekosbestHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NekosbestError.new(
        "graphql_error", "NekosbestSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
