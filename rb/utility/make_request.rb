...
module NekosbestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NekosbestResponse.new({})
    ...
    if fetch_err
      ...
      response = NekosbestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NekosbestResponse.new(fetched)
    else
