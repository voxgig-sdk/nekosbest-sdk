...
class GetRandomByCategoryEntity
  ...
  def make
    opts = @_entopts.dup
    GetRandomByCategoryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NekosbestHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = NekosbestHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GetRandomByCategoryListMatch, Hash, nil] match filter (any subset of
  #   GetRandomByCategory fields); defaults to nil, treated as an empty match that lists all.
