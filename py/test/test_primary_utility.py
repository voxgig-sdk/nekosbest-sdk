# Nekosbest SDK primary utility test
...
from nekosbest_sdk.utility.voxgig_struct import voxgig_struct as vs
from nekosbest_sdk import NekosbestSDK
from nekosbest_sdk.core.spec import NekosbestSpec
from nekosbest_sdk.core.result import NekosbestResult
from nekosbest_sdk.core.response import NekosbestResponse
from nekosbest_sdk.core.operation import NekosbestOperation
from nekosbest_sdk.core.error import NekosbestError
from nekosbest_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NekosbestError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = NekosbestSDK.test(None, None)
        ...
        class TestInitFeature(NekosbestBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NekosbestBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NekosbestSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NekosbestSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NekosbestSDK.test(None, setup_opts)
