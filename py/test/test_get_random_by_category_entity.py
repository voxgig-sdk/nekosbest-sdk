...
class TestGetRandomByCategoryEntity:
    ...
    def test_should_stream(self):
        ...
        base = NekosbestSDK.test(seed, None)
        seen = list(base.GetRandomByCategory(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NekosbestSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.GetRandomByCategory(None).stream("list", None, None):
