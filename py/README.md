...
The Python SDK for the Nekosbest API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetRandomByCategory(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Nekosbest(None).load(
    {"id": "test01"}, None
)
