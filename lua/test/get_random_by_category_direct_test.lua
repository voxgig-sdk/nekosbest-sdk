...
describe("GetRandomByCategoryDirect", function()
  it("should direct-list-get_random_by_category", function()
    local setup = get_random_by_category_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-get_random_by_category", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"get_random_by_category01"}) do
...
function get_random_by_category_direct_setup(mockres)
